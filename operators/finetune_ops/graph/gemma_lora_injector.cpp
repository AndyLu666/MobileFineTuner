...
std::pair<TensorPtr, TensorPtr> create_lora_params(int64_t in_dim, int64_t out_dim, int rank) {
    // 按照 PEFT 默认 init：lora_A 使用 kaiming_uniform_(a=sqrt(5))，lora_B 全零。
    // PyTorch 计算 bound = sqrt(3) * gain / sqrt(fan_in)，gain = sqrt(2/(1+a^2))，a=sqrt(5) -> bound = 1/sqrt(fan_in)
    static std::mt19937 rng(42);
    float bound = 1.0f / std::sqrt(static_cast<float>(in_dim));
    std::uniform_real_distribution<float> dist(-bound, bound);

    // 注意：PEFT 中 lora_A 形状为 [r, in]，lora_B 为 [out, r]；LoRALinear 内部会自动转置到 [in,r]/[r,out]
...
void GemmaLoraInjector::inject(GemmaModel& model, const GemmaLoraSpec& spec) {
    ...
    for (int layer : layers) {
        ...
        for (const auto& module : spec.target_modules) {
            ...
            // 根据模块类型/hidden_size 推断 in/out 方向
...
void GemmaLoraInjector::save_lora_safetensors(const std::string& path) const {
    ...
    // 收集 LoRA A/B（键名沿用 PEFT 風格：layer.{i}.attn.{q|k|v|proj}.lora_{A|B}）
    ...
    auto add_slice = [&](int layer, const std::string& target, const LoRALinear* lin) {
        ...
        for (size_t si = 0; si < slices.size(); ++si) {
            ...
            state[base + ".lora_A"] = sl.A;  // A: [r, in] 或 [in, r] 由 LoRALinear 內部處理，此處保持原樣（PEFT 風格為[r,in]）
            state[base + ".lora_B"] = sl.B;  // B: [out, r] 或 [r, out] 同上
        }
    };
    ...
    for (int i = 0; i < cfg.num_hidden_layers; ++i) {
        ...
        // 若將來啟用 MLP LoRA，可追加如下映射（暫不輸出以避免與其他實現沖突）：
