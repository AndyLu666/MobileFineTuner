...
std::vector<TensorPtr> MatmulBackward::apply(const TensorPtr& grad_output) {
    ...
    if (b_ && b_->requires_grad()) {
        ...
        if (b_shape.size() == 2 && go_shape.size() >= 3) {
            // 批次 matmul 对共享权重的梯度：展平成 [B*, M, K] 与 [B*, M, N]
            ...
            bool weight_in_out = (weight_rows == k && weight_cols == n);   // W形状 [in, out]
            bool weight_out_in = (weight_rows == n && weight_cols == k);   // W形状 [out, in]
...
std::vector<TensorPtr> LinearBackward::apply(const TensorPtr& grad_output) {
    ...
    // 优先匹配权重为 [in_dim, out_dim] 的场景（forward 用 matmul(x, W)）
    ...
    if (weight_cols == grad_out_last_dim && weight_rows == input_last_dim) {
        ...
        // Weight stored as [out_dim, in_dim]; forward等价于 X @ W^T，grad_W 应保持 [out_dim, in_dim]
...
std::vector<TensorPtr> RMSNormBackward::apply(const TensorPtr& grad_output) {
    ...
    for (int64_t b=0;b<batch;++b){
        ...
        // 前向使用 w_eff = 1 + w[i]，這裡反向需同步：y = x_hat * w_eff
...
std::vector<TensorPtr> LoRALinearBackward::apply(const TensorPtr& grad_output) {
    ...
    if (input_->requires_grad()) {
        ...
        // 🔧 穩健處理：有些場景下權重形狀可能被外部系統調換為[output_dim, input_dim]
        // 預期：
        //  - input_.shape()   [..., in_dim]
        //  - grad_output      [..., out_dim]
        //  - weight           [in_dim, out_dim] 或 [out_dim, in_dim]
