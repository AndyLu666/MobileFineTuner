...
void Engine::accumulate_grad(const TensorPtr& tensor, const TensorPtr& grad) {
    ...
    // Ensure the tensor is registered so we can write back grads for leaves/parameters
    auto reg_it = tensor_registry_.find(raw_ptr);
    if (reg_it == tensor_registry_.end()) {
        tensor_registry_[raw_ptr] = tensor;
    }
...
void Engine::run_backward(const std::vector<TensorPtr>& outputs,
    ...
    for (auto& node : topo) {
        ...
        if (!used_legacy) {
            ...
            // Legacy path: 假定 legacy grad_fn_ 已经自行累积到对应输入
            continue;
        }
    }
