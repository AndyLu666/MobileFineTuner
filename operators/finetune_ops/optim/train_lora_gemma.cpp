...
struct CliOptions {
    ...
    // 可选：在对齐模式下从 PT dump 覆盖 LoRA A 权重
    std::string align_pt_weights_dir;
    // 可选：对齐模式下，对 layer0 attn_out_raw 做有限差分数值梯度检查
    bool align_numeric_attn = false;
    float align_numeric_eps = 1e-3f;
    int align_numeric_count = 4;  // 每个目标取前几个元素
    std::string align_numeric_targets = "attn_out_raw_l0";
    std::string target_mode = "full";
    std::string lora_targets_override;  // 逗号分隔，非空时覆盖 target_mode 默认列表
    int epochs = 1;
    int max_steps = -1;
    int seq_len = 256;
    int batch = 4;
    ...
    std::string loss_reduction = "mean";  // 对齐调试时可改为 sum_debug
...
CliOptions parse_cli(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg.rfind("--model_dir", 0) == 0) {
            ...
            opts.lora_targets_override = get_val("--lora_targets");  // 例如 "q_proj,k_proj"
...
int main(int argc, char** argv) {
    try {
        ...
        if (!cli.lora_targets_override.empty()) {
            // 解析逗号分隔的目标，覆盖默认模块列表；用于对齐/调试只挂单个或部分 LoRA。
        ...
        if (!cli.align_dump_dir.empty()) {
            lora_spec.dropout = 0.0f;  // 对齐时关闭dropout
        }
        GemmaLoraInjector injector;
        injector.inject(model, lora_spec);
        injector.print_info();

        // 可选：在对齐模式下用 PT dump 覆盖 LoRA A 初始权重（验证随机流差异）
        ...
        if (!cli.align_dump_dir.empty()) {
            ...
            logits->set_requires_grad(true);  // 便于对齐时dump dlogits
            logits->retain_grad();
            ...
            save_npy(cli.align_dump_dir + "/per_token_nll.npy",
                     ...
                     DumpDType::kFloat32);
            // loss_scalar 直接使用 lm_cross_entropy(带 reduction) 的输出，保持与 PT 基线一致。
            ...
            if (cli.align_dump_grads) {
                ...
                if (!cli.align_disable_debug) {
                    ...
                    // 解析导出：dMat/dQ/dK/dV（对每个 layer 进行显式计算并保存到 grads/analytical_*）
                    {
                        ...
                        for (int layer : target_layers) {
                            ...
                            // dV_heads = probs^T @ G_context (按head还原)
                            if (ctx && ctx->grad()) {
                    ...
                    if (cli.align_numeric_attn) {
                        auto debug_map_base = model.debug_tensors();  // 基础前向的grad
        ...
        std::cout << "🎉 Gemma LoRA 训练完成" << std::endl;
        return 0;
