...
static Args parse_args(int argc, char** argv) {
    Args a;
    a.data_root = "/Users/tony/Documents/重新开始/data/wikitext2/wikitext-2-raw";
    a.pretrained_dir = "/Users/tony/Documents/重新开始/gpt2_lora_finetune/pretrained/gpt2";
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        try {
            ...
            cout << "  ✓ GPT-2 config loaded: layers=" << cfg.n_layer
                 << ", hidden=" << cfg.n_embd
                 << ", heads=" << cfg.n_head << endl;
        } catch (const std::exception& e) {
            cout << "  ⚠️ Failed to read config, fallback to default: " << e.what() << endl;
        }
        if (args.seq_len > cfg.n_positions) {
            cout << "  ⚠️ seq_len clipped from " << args.seq_len << " to " << cfg.n_positions << endl;
        ...
        // GPT-2 safetensors 线性层权重为 [in, out]，与模型内部一致，无需转置
        ...
        if (!args.lora_path.empty()) {
            ...
            if (!lora_state.compatible_with(model)) {
                cerr << "❌ LoRA incompatible with model" << endl;
                return 1;
            }
            
            // 初始化LoRALinear模块（不inject，直接attach加载的权重）
        ...
        if (!args.out_file.empty()) {
            ...
            if (out) {
                ...
                out.close();
                cout << "✅ Wrote results to " << args.out_file << endl;
            }
        }
