...
void MemoryPool::clear_unused() {
    ...
    // Only log if something unexpected happens (memory leak indicators)
    if (freed_bytes > 1024 * 1024 * 1024) { // Only log if >1GB freed (indicates potential leak)
        OPS_LOG_ERROR_F("Large memory cleanup: freed %zu MB from %zu blocks (potential memory leak)", 
                         freed_bytes / (1024 * 1024), freed_count);
    }
    // Normal cleanup is silent - working as expected
}
