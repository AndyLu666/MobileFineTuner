 ...
 * @brief Extreme memory-saving attention mechanism implementation
 */
...
TensorPtr memory_first_attention_forward(
    ...
    // Process independently for each batch and head
    for (int64_t b = 0; b < batch; ++b) {
        for (int64_t h = 0; h < num_heads; ++h) {
            // Data start position of current head
            int64_t head_offset = (b * seq_len * num_heads + h) * head_dim;
            int64_t stride_seq = num_heads * head_dim;
            
            // Allocate temporary score block [row_block, col_block]
            std::vector<float> scores_block(row_block_size * col_block_size);
            
            // Iterate over Query row blocks
            for (int64_t row_start = 0; row_start < seq_len; row_start += row_block_size) {
                ...
                // Initialize online softmax state for each row in current row block
                ...
                // Iterate over Key/Value column blocks
                for (int64_t col_start = 0; col_start < seq_len; col_start += col_block_size) {
                    ...
                    // Compute score block Sij = Qi @ Kj^T * scale
                    ...
                    // Update online softmax state with current score block and corresponding Value block
                    for (int64_t i = 0; i < actual_row_block; ++i) {
                        const float* row_scores = &scores_block[i * actual_col_block];
                        
                        // Collect Values corresponding to current column block [actual_col_block, head_dim]
    ...
    // Set gradient computation (simplified version: mark as requiring gradient but don't implement backward)
...
TensorPtr memory_first_multihead_attention(
    ...
    // Extreme memory-saving attention computation
    auto attn_out = memory_first_attention_forward(
        q_reshaped, k_reshaped, v_reshaped, causal, 32, 32
    );
    
    // Reshape back: [batch, seq_len, num_heads, head_dim] -> [batch, seq_len, n_embd]
