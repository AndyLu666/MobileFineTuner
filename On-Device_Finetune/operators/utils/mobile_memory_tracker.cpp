...
MobileMemoryTracker::MobileMemoryTracker(const std::string& output_dir) 
    ...
    if (csv_file_.is_open()) {
        write_csv_header();
        std::cout << "[MemoryTracker] CSV: " << csv_path << std::endl;
    }
    ...
    if (json_file_.is_open()) {
        json_file_ << "{\n  \"memory_snapshots\": [\n";
        std::cout << "[MemoryTracker] JSON: " << json_path << std::endl;
    }
    ...
    std::cout << "[MemoryTracker] initializationcompleted， RSS: " 
              << (baseline_rss_ / 1024.0 / 1024.0) << " MB" << std::endl;
}
...
void MobileMemoryTracker::record_step(int epoch, int step, float loss, float grad_norm, long step_time_ms) {
    ...
    // memorywarning
    if (leak_detected_ && step % 50 == 0) {
        std::cout << "\n⚠️  [MemoryTracker] [Output]memory[Output]，[Output]！" << std::endl;
        std::cout << "    current RSS: " << (snapshot.rss_bytes / 1024.0 / 1024.0) << " MB" << std::endl;
        std::cout << "     RSS: " << (peak_rss_ever_ / 1024.0 / 1024.0) << " MB" << std::endl;
        std::cout << "    : " << std::fixed << std::setprecision(2) 
                  << get_memory_growth_rate() << " MB/step\n" << std::endl;
    }
}
